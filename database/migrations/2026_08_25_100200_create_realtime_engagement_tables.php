...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('webinar_reactions', function (Blueprint $table) {
            $table->id();
            $table->foreignId('webinar_id')->constrained()->cascadeOnDelete();
            $table->foreignId('user_id')->constrained()->cascadeOnDelete();
            $table->string('reaction', 30);
            $table->timestamp('reacted_at')->useCurrent()->index();
        });
    }

    public function down(): void
    {
        foreach (['webinar_reactions', 'poll_responses', 'poll_options', 'polls', 'question_votes', 'question_answers', 'questions', 'chat_messages'] as $table) {
