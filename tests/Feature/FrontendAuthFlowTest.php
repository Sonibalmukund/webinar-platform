...
class FrontendAuthFlowTest extends TestCase
{
    ...
    public function test_staff_keep_their_dedicated_login_pages(): void
    {
        $this->get('/admin/dashboard')->assertRedirect(route('admin.login'));
        $this->get('/sub-admin/dashboard')->assertRedirect(route('subadmin.login'));
        $this->get('/admin/login')->assertOk()->assertSee('Sign in securely');
        $this->get('/sub-admin/login')->assertOk()->assertSee('Sign in securely');
    }
    ...
    public function test_event_login_returns_to_landing_with_success_toast(): void
    {
        $webinar = $this->webinar();
        $user = $this->learner();
        ...
        $this->actingAs($user)->withSession(['auth_status'=>'Login successfully.'])->get(route('webinars.show',$webinar))
    ...
    public function test_unregistered_learner_returns_to_event_registration_section(): void
    {
        $webinar = $this->webinar();
        $user = $this->learner();
        $this->post('/login', ['return_to'=>'/'.$webinar->slug,'webinar_id'=>$webinar->id,'login'=>$user->email,'password'=>'PopupPass123'])
            ->assertRedirect(route('webinars.show',$webinar).'#registration');
    }
    ...
    public function test_registration_from_popup_creates_event_seat_and_returns_to_landing(): void
    {
        ...
        ])->assertRedirect(route('webinars.show',$webinar))->assertSessionHas('registration_status');
