...
class SessionResourcesTest extends TestCase
{
    ...
    private function user(string $role): User { $user=User::factory()->create();$user->roles()->attach(Role::firstOrCreate(['slug'=>$role],['name'=>$role]));return $user; }

    public function test_admin_can_save_resources_and_learner_sees_them_in_room_and_landing(): void
    {
        $admin=$this->user('super-admin');$learner=$this->user('learner');
