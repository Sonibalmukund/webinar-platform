...
class AuthenticationTest extends TestCase
{
    ...
    {
        $country = \App\Models\Country::where('iso2', 'IN')->firstOrFail();
        $state = \App\Models\State::where('country_id', $country->id)->where('name', 'Gujarat')->firstOrFail();
        $city = \App\Models\City::where('state_id', $state->id)->firstOrFail();
        $requiredField = \App\Models\SignupField::where('is_required', true)->first();
        $this->post('/register', [
    ...
    public function test_learner_can_reserve_a_webinar_seat(): void
    {
        ...
        $webinar = \App\Models\Webinar::updateOrCreate(['slug' => 'test-webinar'], ['created_by' => $admin->id, 'title' => 'Test Webinar']);
    ...
    {
        $india = \App\Models\Country::where('iso2','IN')->firstOrFail();
        $gujarat = \App\Models\State::where('country_id',$india->id)->where('name','Gujarat')->firstOrFail();
        ...
        $field = \App\Models\SignupField::where('is_required',true)->first();
        $this->post('/register', [
    ...
    public function test_admin_can_save_webinar_registration_fields(): void
    {
        ...
        $webinar=\App\Models\Webinar::where('title','Dynamic Registration Webinar')->firstOrFail();
    ...
    public function test_learner_can_login_with_mobile_number_and_password(): void
    {
        $role=Role::firstOrCreate(['slug'=>'learner'],['name'=>'Learner']);
        $user=User::updateOrCreate(['email'=>'mobile.login@example.com'],['name'=>'Mobile Learner','mobile'=>'+919876543210','password'=>'Webinar@123']);
        $user->roles()->sync([$role->id]);
        $this->post('/login',['login'=>'+919876543210','password'=>'Webinar@123'])->assertRedirect('/dashboard');
    }
    ...
    public function test_webinar_landing_page_always_shows_guest_login_button(): void
    {
        ...
        $admin->roles()->sync([$adminRole->id]);
        $webinar=\App\Models\Webinar::updateOrCreate(['slug'=>'landing-login-test'],['created_by'=>$admin->id,'title'=>'Landing Login Test','status'=>'scheduled']);
    ...
    public function test_learner_dashboard_engagement_and_certificate_actions_work(): void
    {
        ...
        $webinar=\App\Models\Webinar::updateOrCreate(['slug'=>'engagement-actions-test'],['created_by'=>$admin->id,'title'=>'Engagement Actions','status'=>'live','chat_enabled'=>true,'polls_enabled'=>true,'comments_enabled'=>true,'feedback_enabled'=>true,'certificate_enabled'=>'yes']);
        \App\Models\Webinar::updateOrCreate(['slug'=>'not-my-webinar-test'],['created_by'=>$admin->id,'title'=>'Not My Webinar','status'=>'scheduled']);
        \App\Models\Registration::updateOrCreate(['webinar_id'=>$webinar->id,'user_id'=>$learner->id],['email'=>$learner->email,'status'=>'approved','registered_at'=>now()]);
        $poll=\App\Models\Poll::create(['webinar_id'=>$webinar->id,'created_by'=>$admin->id,'question'=>'Was this useful?','status'=>'active']);
        ...
        $this->assertDatabaseHas('feedback',['webinar_id'=>$webinar->id,'user_id'=>$learner->id,'rating'=>5]);
        ...
        \Illuminate\Support\Facades\DB::table('certificates')->insert(['webinar_id'=>$webinar->id,'user_id'=>$learner->id,'credential_id'=>(string)\Illuminate\Support\Str::uuid(),'status'=>'approved','issued_at'=>now(),'created_at'=>now(),'updated_at'=>now()]);
        ...
        $this->post('/login',['login'=>$learner->email,'password'=>'Webinar@123','return_to'=>'/'.$webinar->slug])
            ->assertRedirect(route('webinars.show',$webinar))->assertSessionHas('auth_status');
    }
}
