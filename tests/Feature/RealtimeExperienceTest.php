...
class RealtimeExperienceTest extends TestCase
{
    ...
    public function test_attendance_join_heartbeat_and_leave_record_real_watch_time(): void
    {
        ...
        $admin=$this->user('super-admin');$learner=$this->user('learner');$webinar=$this->webinar($admin);
    ...
    {
        $admin=$this->user('super-admin');$webinar=$this->webinar($admin);
    ...
    public function test_admin_notification_is_saved_and_broadcast_to_each_learner(): void
    {
        ...
        $admin=$this->user('super-admin');$one=$this->user('learner');$two=$this->user('learner');$webinar=$this->webinar($admin);
        foreach([$one,$two] as $learner) Registration::create(['webinar_id'=>$webinar->id,'user_id'=>$learner->id,'email'=>$learner->email,'status'=>'approved']);
    ...
    {
        $admin=$this->user('super-admin');$sub=$this->user('sub-admin');$assigned=$this->webinar($admin);$other=$this->webinar($admin);
        $sub->assignedWebinars()->attach($assigned->id,['assigned_by'=>$admin->id]);
        $this->actingAs($sub)->get(route('subadmin.dashboard'))->assertOk()->assertSee('ASSIGNED EVENTS');
        $this->actingAs($sub)->get(route('subadmin.chats.show',$assigned))->assertOk()
            ->assertSee('data-chat-history="'.route('subadmin.chats.show',$assigned).'"',false);
        $this->get(route('subadmin.chats.show',$other))->assertForbidden();
    }
}
