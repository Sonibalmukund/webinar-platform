...
class DashboardController extends Controller
{
    ...
    public function admin(Request $request): View
    {
        ...
        $chart = $days->map(function ($day) use ($registrations, $attendance) {
            return ['label' => $day->format('D'), 'registrations' => $registrations->filter(fn ($row) => ($row->registered_at ?? $row->created_at)?->isSameDay($day))->count(), 'attendees' => $attendance->filter(fn ($row) => $row->joined_at && Carbon::parse($row->joined_at)->isSameDay($day))->count()];
        });
