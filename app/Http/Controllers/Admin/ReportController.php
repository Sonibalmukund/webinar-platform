...
class ReportController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $performance=$webinars->whereIn('id',$webinarIds)->map(function($webinar) use($registrations,$attendees,$polls,$votes){ $eventPolls=$polls->where('webinar_id',$webinar->id); $registered=$registrations->where('webinar_id',$webinar->id)->count(); $attended=$attendees->where('webinar_id',$webinar->id)->count(); return ['webinar'=>$webinar,'registered'=>$registered,'attended'=>$attended,'rate'=>$registered?round($attended/$registered*100):0,'polls'=>$eventPolls->count(),'votes'=>$votes->whereIn('poll_id',$eventPolls->pluck('id'))->count(),'watch'=>(int)$attendees->where('webinar_id',$webinar->id)->sum('watch_seconds')]; });
