...
class WebinarController extends Controller
{
    ...
    {
        $webinars=Webinar::with('registrationForm')->withCount('registrations')->latest()->paginate(15);$webinars->getCollection()->each(fn($webinar)=>$webinar->health=WebinarHealth::score($webinar));return view('pages.admin.webinars.index',compact('webinars'));
    }

    public function create(): View { return view('pages.admin.webinars.form', ['webinar'=>new Webinar,'sessionResourcesText'=>'']); }
    ...
    public function store(Request $request): RedirectResponse
    {
        $webinar = DB::transaction(function () use ($request) {
            ...
            if ($request->hasAny(['registration_enabled', 'fields'])) $this->saveRegistrationForm($request, $webinar);
    ...
    public function edit(Webinar $webinar): View
    {
        ...
        return view('pages.admin.webinars.form', ['webinar'=>$webinar->load('registrationForm.fields.options'),'sessionResourcesText'=>$sessionResourcesText]);
    }

    public function show(Webinar $webinar): View
    {
        $webinar=$webinar->load(['registrationForm.fields','speakers','polls'])->loadCount('registrations');$readiness=['Schedule'=>(bool)($webinar->starts_at&&$webinar->ends_at),'Video player'=>(bool)$webinar->live_url,'Registration form'=>(bool)$webinar->registrationForm?->is_active,'Speaker assigned'=>$webinar->speakers->isNotEmpty(),'Poll configured'=>$webinar->polls->isNotEmpty(),'Certificate'=>(bool)data_get($webinar->settings,'certificate_template_id')];$events=DB::table('webinar_attendance_events')->where('webinar_id',$webinar->id)->where('event_type','heartbeat')->pluck('occurred_at')->groupBy(fn($at)=>Carbon::parse($at)->format('H'))->map(fn($items,$hour)=>(object)['hour'=>$hour,'total'=>$items->count()])->values();return view('pages.admin.webinars.show',compact('webinar','readiness','events'));
    }
    ...
    {
        $webinar->update(['status'=>$request->input('status',$webinar->status),'chat_enabled'=>$request->boolean('chat_enabled'),'qa_enabled'=>$request->boolean('qa_enabled'),'polls_enabled'=>$request->boolean('polls_enabled')]);
    ...
    {
        $data=$request->validate(['status'=>['required','in:draft,scheduled,live,completed,cancelled']]);if(in_array($data['status'],['scheduled','live'])&&(!$webinar->starts_at||!$webinar->ends_at))return back()->withErrors(['status'=>'Set the start and end time before publishing this webinar.']);$webinar->update(['status'=>$data['status'],'published_at'=>$data['status']==='draft'?null:($webinar->published_at??now())]);$this->broadcastRoom($webinar,'status');AuditTrail::record('webinar.status',$webinar,'Webinar status changed to '.$data['status'].'.');return back()->with('status','Webinar status updated.');
    }
    ...
    {
        $copy=DB::transaction(function()use($webinar){$webinar->load(['registrationForm.fields.options','polls.options','speakers']);$copy=$webinar->replicate(['slug','published_at']);$copy->title=$webinar->title.' Copy';$copy->slug=Str::slug($copy->title).'-'.Str::lower(Str::random(6));$copy->status='draft';$copy->published_at=null;$copy->starts_at=null;$copy->ends_at=null;$copy->save();if($webinar->registrationForm){$form=$webinar->registrationForm->replicate();$form->webinar_id=$copy->id;$form->save();$fieldMap=[];foreach($webinar->registrationForm->fields as $field){$new=$field->replicate(['condition_field_id']);$new->registration_form_id=$form->id;$new->save();$fieldMap[$field->id]=$new->id;foreach($field->options as $option){$newOption=$option->replicate();$newOption->registration_field_id=$new->id;$newOption->save();}}foreach($webinar->registrationForm->fields as $field)if($field->condition_field_id&&!empty($fieldMap[$field->condition_field_id]))$form->fields()->whereKey($fieldMap[$field->id])->update(['condition_field_id'=>$fieldMap[$field->condition_field_id]]);}foreach($webinar->polls as $poll){$newPoll=$poll->replicate(['started_at','ended_at']);$newPoll->webinar_id=$copy->id;$newPoll->status='draft';$newPoll->save();foreach($poll->options as $option){$newOption=$option->replicate();$newOption->poll_id=$newPoll->id;$newOption->save();}}$copy->speakers()->sync($webinar->speakers->mapWithKeys(fn($speaker)=>[$speaker->id=>['role'=>$speaker->pivot->role,'display_order'=>$speaker->pivot->display_order]])->all());foreach(Banner::where('webinar_id',$webinar->id)->get() as $item){$new=$item->replicate(['starts_at','ends_at']);$new->webinar_id=$copy->id;$new->is_active=false;$new->save();}foreach(Brand::where('webinar_id',$webinar->id)->get() as $item){$new=$item->replicate();$new->webinar_id=$copy->id;$new->save();}return $copy;});
    ...
    private function webinarData(Request $request, ?Webinar $webinar=null): array
    {
        ...
        if($provider && filled($player['live_source']??null) && !$data['live_url']) throw \Illuminate\Validation\ValidationException::withMessages(['live_source'=>'Enter a valid '.$provider.' video URL, code, or iframe.']);
        foreach(['starts_at','ends_at'] as $field) if(filled($data[$field]??null)) $data[$field]=Carbon::parse($data[$field],$data['timezone'])->utc();
        ...
        $data['qa_enabled']=$request->boolean('qa_enabled');
        ...
        if ($data['registration_type'] === 'free') $data['price'] = null;
        ...
        $settings['experience']=[
            ...
            'chapters'=>collect(preg_split('/\r\n|\r|\n/',$experience['video_chapters']??''))->filter()->map(function($line){[$time,$title]=array_pad(explode('|',$line,2),2,'');return ['time'=>trim($time),'title'=>trim($title)];})->values()->all(),
    ...
    private function uniqueSlug(string $title, ?int $ignoreId=null): string
    {
        ...
        $slug=$base;
        $suffix=2;
        while(Webinar::where('slug',$slug)->when($ignoreId,fn($query)=>$query->where('id','!=',$ignoreId))->exists()) $slug=$base.'-'.$suffix++;
        return $slug;
    }
    ...
    private function saveRegistrationForm(Request $request, Webinar $webinar): void
    {
        ...
        foreach ($request->input('fields',[]) as $order=>$row) {
            if (blank($row['label']??null)) continue;
            ...
            if (in_array($field->field_type,['dropdown','radio','checkbox'])) foreach (preg_split('/\r\n|\r|\n/',trim($row['options']??'')) as $i=>$label) if(filled($label)) $field->options()->create(['label'=>trim($label),'value'=>Str::slug(trim($label),'_'),'display_order'=>$i,'is_enabled'=>true]);
        }
    ...
    private function saveLegacyPollsAndCertificate(Request $request, Webinar $webinar): void
    {
        if ($request->has('polls')) {
            ...
            foreach ($request->input('polls', []) as $row) {
                if (blank($row['question'] ?? null)) continue;
                ...
                foreach (preg_split('/\r\n|\r|\n/', trim($row['options'] ?? '')) as $order => $label) {
                    if (filled($label)) $poll->options()->create(['label' => trim($label), 'display_order' => $order]);
                }
            }
    ...
    {
        try { broadcast(new WebinarRoomUpdated($webinar->id,$change,['status'=>$webinar->status,'chat_enabled'=>$webinar->chat_enabled,'polls_enabled'=>$webinar->polls_enabled])); } catch(\Throwable $e) { report($e); }
    }

    private function saveSessionResources(Request $request,Webinar $webinar): void
    {
        if(!$request->has('session_resources')) return;
        $lines=collect(preg_split('/\r\n|\r|\n/',trim((string)$request->input('session_resources'))))->filter();
        DB::table('webinar_resources')->where('webinar_id',$webinar->id)->delete();
        foreach($lines->values() as $order=>$line){[$title,$url]=array_pad(array_map('trim',explode('|',$line,2)),2,'');$safeUrl=Str::startsWith($url,'/')||filter_var($url,FILTER_VALIDATE_URL)&&in_array(parse_url($url,PHP_URL_SCHEME),['http','https'],true);if($title===''||!$safeUrl)continue;DB::table('webinar_resources')->insert(['webinar_id'=>$webinar->id,'uploaded_by'=>$request->user()->id,'title'=>$title,'type'=>'link','path_or_url'=>$url,'is_public'=>true,'display_order'=>$order,'created_at'=>now(),'updated_at'=>now()]);}
    }
}
