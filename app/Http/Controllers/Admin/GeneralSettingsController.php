...
class GeneralSettingsController extends Controller
{
    ...
    {
        return view('pages.admin.general.site', ['settings' => DB::table('settings')->where('group', 'site')->pluck('value', 'key')]);
    }

    public function updateSite(Request $request): RedirectResponse
    {
        $data = $request->validate(['site_name' => ['required', 'string', 'max:100'], 'footer_text' => ['required', 'string', 'max:500'], 'admin_email' => ['required', 'email'], 'site_logo' => ['nullable', 'image', 'max:5120'], 'small_logo' => ['nullable', 'image', 'max:2048'], 'favicon' => ['nullable', 'image', 'max:1024']]);
        foreach (['site_logo', 'small_logo', 'favicon'] as $key) {
            if ($request->hasFile($key)) {
                $data[$key] = $this->upload($request->file($key), 'site');
            }
        }foreach ($data as $key => $value) {
            DB::table('settings')->updateOrInsert(['key' => $key], ['group' => 'site', 'value' => $value, 'is_public' => true, 'created_at' => now(), 'updated_at' => now()]);
        }
    ...
    public function bulkDeleteBanners(Request $request): RedirectResponse
    {
        $ids = $request->validate(['ids' => ['required', 'array', 'min:1'], 'ids.*' => ['integer', 'exists:banners,id']])['ids'];
        Banner::whereIn('id', $ids)->delete();

        return back()->with('status', count($ids).' banners deleted.');
    }
    ...
    public function toggleBanner(Banner $banner): RedirectResponse
    {
        ...
        return back()->with('status', 'Banner status updated.');
    }

    public function reorderBanners(Request $request): RedirectResponse
    {
        $ids = $request->validate(['order' => ['required', 'array'], 'order.*' => ['integer', 'exists:banners,id']])['order'];
        foreach ($ids as $index => $id) {
            Banner::whereKey($id)->update(['display_order' => $index]);
        }
    ...
    public function bulkDeleteBrands(Request $request): RedirectResponse
    {
        $ids = $request->validate(['ids' => ['required', 'array', 'min:1'], 'ids.*' => ['integer', 'exists:brands,id']])['ids'];
        Brand::whereIn('id', $ids)->delete();

        return back()->with('status', count($ids).' brands deleted.');
    }
    ...
    public function toggleBrand(Brand $brand): RedirectResponse
    {
        ...
        return back()->with('status', 'Brand status updated.');
    }
