...
class RegistrationSettingsController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $selectedWebinar=$request->filled('webinar_id')?Webinar::with('registrationForm.fields.options')->findOrFail($request->integer('webinar_id')):null;
        return view('pages.admin.registration-settings', [
            ...
            'webinars'=>Webinar::with('registrationForm')->latest()->get(),
    ...
    {
        if (!$request->boolean('email_enabled') && !$request->boolean('mobile_enabled')) return back()->withErrors(['email_enabled'=>'Enable at least Email or Mobile.'])->withInput();
        ...
        foreach ($values as $key=>$value) DB::table('settings')->updateOrInsert(['key'=>$key], ['group'=>'registration','value'=>$value,'is_public'=>false,'created_at'=>now(),'updated_at'=>now()]);
    ...
    public function storeWebinarField(Request $request, Webinar $webinar): RedirectResponse
    {
        ...
        if($request->boolean('login_enabled')) $form->fields()->update(['login_enabled'=>false]);
        ...
        if(in_array($field->field_type,['dropdown','radio','checkbox'])) foreach(array_values(array_filter(array_map('trim',preg_split('/\r\n|\r|\n/',$data['options']??'')))) as $order=>$label) $field->options()->create(['label'=>$label,'value'=>Str::slug($label,'_'),'display_order'=>$order,'is_enabled'=>true]);
        return redirect()->route('admin.registration-settings',['webinar_id'=>$webinar->id])->with('status','Dynamic field added to '.$webinar->title.'.');
    }
    ...
    public function updateWebinarField(Request $request, RegistrationField $field): RedirectResponse
    {
        ...
        if($request->boolean('login_enabled')) $field->form->fields()->whereKeyNot($field->id)->update(['login_enabled'=>false]);
        ...
        if(in_array($field->field_type,['dropdown','radio','checkbox'])) foreach(array_values(array_filter(array_map('trim',preg_split('/\r\n|\r|\n/',$data['options']??'')))) as $order=>$label) $field->options()->create(['label'=>$label,'value'=>Str::slug($label,'_'),'display_order'=>$order,'is_enabled'=>true]);
        return redirect()->route('admin.registration-settings',['webinar_id'=>$field->form->webinar_id])->with('status','Dynamic field updated.');
    }
    ...
    {
        $field->update(['is_enabled'=>!$field->is_enabled]); return back()->with('status','Webinar field visibility updated.');
    }

    public function destroyWebinarField(RegistrationField $field): RedirectResponse
    {
        $field->delete(); return back()->with('status','Webinar field deleted.');
    }
    ...
    public function applyPreset(Request $request, Webinar $webinar): RedirectResponse
    {
        ...
        $form=$webinar->registrationForm()->firstOrCreate([],['title'=>'Registration Form','is_active'=>true,'require_login'=>true]);
        foreach($sets[$preset] as $item){$key=Str::slug($item[0],'_');$form->fields()->firstOrCreate(['field_key'=>$key],['label'=>$item[0],'field_type'=>'text','placeholder'=>'Enter '.Str::lower($item[0]),'icon'=>$item[1],'width'=>'half','is_required'=>false,'is_enabled'=>true,'display_order'=>($form->fields()->max('display_order')??0)+1]);}
