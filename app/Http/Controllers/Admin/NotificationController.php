...
class NotificationController extends Controller
{
    ...
    {
        return view('pages.admin.notifications.create', ['webinars' => Webinar::orderBy('title')->get(['id', 'title'])]);
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        foreach ($users as $user) {
            $id = (string) Str::uuid();
            $payload = ['id' => $id, 'campaign_id' => $campaign, 'subject' => $data['subject'], 'message' => $data['message'], 'webinar_id' => $data['webinar_id'] ?? null, 'created_at' => now()->toIso8601String()];
            DB::table('user_notifications')->insert(['id' => $id, 'user_id' => $user->id, 'type' => 'campaign', 'data' => json_encode($payload), 'created_at' => now(), 'updated_at' => now()]);
            try {
                broadcast(new UserNotificationCreated($user->id, $payload));
            } catch (\Throwable $e) {
                report($e);
            }
        }
