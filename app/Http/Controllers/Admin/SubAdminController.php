...
class SubAdminController extends Controller
{
    ...
    {
        abort_unless($subAdmin->hasRole('sub-admin'), 404);

        return $this->form($subAdmin->load('assignedWebinars'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('admin.subadmins.index')->with('status', 'Sub admin created successfully.');
    }

    public function update(Request $request, User $subAdmin): RedirectResponse
    {
        abort_unless($subAdmin->hasRole('sub-admin'), 404);
        $this->save($request, $subAdmin);

        return redirect()->route('admin.subadmins.index')->with('status', 'Sub admin updated successfully.');
    }
    ...
    public function destroy(User $subAdmin): RedirectResponse
    {
        ...
        return back()->with('status', 'Sub admin deleted.');
    }
}
