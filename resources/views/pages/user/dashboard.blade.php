    ...
    <div class="webinar-schedule-list">
        @forelse($scheduledWebinars as $registration)
            ...
            <article class="webinar-schedule-row">
                ...
                <div class="schedule-actions"><span class="status-badge {{ $registration->status==='approved'?'active':'scheduled' }}">{{ ucfirst($registration->status) }}</span><a class="btn btn-sm btn-gradient" href="{{ route('webinars.dashboard',$scheduled) }}">Open dashboard <i class="bi bi-arrow-right"></i></a></div>
            </article>
        @empty
