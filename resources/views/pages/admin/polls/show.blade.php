...
<section class="panel-card poll-results-card">
    ...
    @if($poll->type==='Quiz')<div class="alert alert-success"><strong>Correct answer:</strong> {{ $poll->options->firstWhere('is_correct',true)?->label }} · Correct: {{ $correctVotes }} · Incorrect: {{ $totalVotes-$correctVotes }} · Accuracy: {{ $totalVotes?round($correctVotes/$totalVotes*100,1):0 }}%</div>@endif
    <div class="poll-result-chart">
        @forelse($poll->options as $option)
            ...
            <div class="poll-result-row"><div class="poll-result-label"><strong>{{ $option->label }} @if($option->is_correct)<i class="bi bi-check-circle-fill text-success"></i>@endif</strong><span>{{ $option->responses_count }} users · {{ $percentage }}%</span></div><div class="poll-result-track"><span style="width: {{ $percentage }}%"></span></div></div>
        @empty
