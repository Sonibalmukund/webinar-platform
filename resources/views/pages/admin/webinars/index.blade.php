...
<div class="page-heading"><div><span class="eyebrow">ADMIN WORKSPACE</span><h1>Webinars</h1><p>Every webinar has its own public client URL.</p></div>@if($isSuperAdmin || auth()->user()->hasPermission('webinars.create'))<a class="btn btn-gradient" href="{{ route('admin.webinars.create') }}"><i class="bi bi-plus-lg"></i> Create webinar</a>@endif</div>
...
<td>@if($isSuperAdmin || auth()->user()->hasPermission('webinars.edit'))<form class="webinar-status-form {{ $webinar->status }}" method="POST" action="{{ route('admin.webinars.status',$webinar) }}" data-no-validation>@csrf @method('PATCH')<i></i><select class="webinar-status-select" name="status" onchange="this.form.submit()" aria-label="Change webinar status">@foreach(['draft','scheduled','live','completed','cancelled'] as $status)<option value="{{ $status }}" @selected($webinar->status===$status)>{{ ucfirst($status) }}</option>@endforeach</select><span class="bi bi-chevron-down"></span></form>@else<span class="status-badge {{ $webinar->status }}">{{ strtoupper($webinar->status) }}</span>@endif</td>
<td><div class="dropdown"><button class="icon-btn" data-bs-toggle="dropdown" aria-label="Actions"><i class="bi bi-three-dots"></i></button><ul class="dropdown-menu dropdown-menu-end shadow-sm border-0">@if($isSuperAdmin || auth()->user()->hasPermission('live-control.view'))<li><a class="dropdown-item text-danger fw-bold d-flex align-items-center" href="{{ route('admin.webinars.live',$webinar) }}"><i class="bi bi-broadcast me-2"></i>Live control</a></li>@endif<li><a class="dropdown-item d-flex align-items-center" href="{{ route('admin.webinars.show',$webinar) }}"><i class="bi bi-eye me-2"></i>View overview</a></li>@if($isSuperAdmin || auth()->user()->hasPermission('webinars.edit'))<li><a class="dropdown-item d-flex align-items-center" href="{{ route('admin.webinars.edit',$webinar) }}"><i class="bi bi-pencil me-2"></i>Edit webinar</a></li>@endif @if($isSuperAdmin || auth()->user()->hasPermission('webinars.create'))<li><form method="POST" action="{{ route('admin.webinars.clone',$webinar) }}">@csrf<button class="dropdown-item d-flex align-items-center"><i class="bi bi-copy me-2"></i>Clone as draft</button></form></li>@endif @if($isSuperAdmin || auth()->user()->hasPermission('webinars.delete'))<li><hr class="dropdown-divider"></li><li><form method="POST" action="{{ route('admin.webinars.destroy',$webinar) }}" onsubmit="return confirm('Delete this webinar?')">@csrf @method('DELETE')<button class="dropdown-item text-danger d-flex align-items-center"><i class="bi bi-trash me-2"></i>Delete</button></form></li>@endif</ul></div></td>
</tr>
