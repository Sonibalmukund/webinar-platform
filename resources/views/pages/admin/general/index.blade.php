...
@php($isSuperAdmin=auth()->user()->hasRole('super-admin'))
@php($canCreate=$type!=='speaker'||$isSuperAdmin||auth()->user()->hasPermission('speakers.create'))
@php($canEdit=$type!=='speaker'||$isSuperAdmin||auth()->user()->hasPermission('speakers.edit'))
@php($canDelete=$type!=='speaker'||$isSuperAdmin||auth()->user()->hasPermission('speakers.delete'))
...
<div class="page-heading"><div><span class="eyebrow">{{ $type === 'speaker' ? 'WEBINAR MANAGEMENT' : 'GENERAL SETTINGS' }}</span><h1>{{ ucfirst($type) }}s</h1><p>Manage webinar {{ $type }}s, previews and visibility.</p></div>@if($canCreate)<a class="btn btn-gradient" href="{{ route($routeBase.'.create') }}"><i class="bi bi-plus"></i> Add {{ $type }}</a>@endif</div>
...
<form class="filter-bar module-filter-bar mb-4" method="GET">
    ...
    @if(isset($webinars) && $webinars->isNotEmpty() && !auth()->user()->hasRole('sub-admin'))
...
@if($canDelete)<div class="bulk-toolbar" data-bulk-toolbar><strong><span data-selected-count>0</span> selected</strong><span class="text-muted">Choose the items you want to remove.</span><button type="submit" form="bulkDeleteForm" class="btn btn-sm btn-danger ms-auto"><i class="bi bi-trash"></i> Delete selected</button></div>@endif
...
<td>@if($media)@php(preg_match('/(?:youtu\.be\/|youtube(?:-nocookie)?\.com\/(?:watch\?v=|embed\/|shorts\/|live\/))([A-Za-z0-9_-]{11})/i',(string)$media,$ytListMatch))@php($isVideo=$type==='banner'&&$row->media_type==='video')@php($isYt=$isVideo&&!empty($ytListMatch[1]))@php($mType=$isYt?'youtube':($isVideo?'video':'image'))<button type="button" class="btn p-0 border-0 bg-transparent text-start position-relative d-inline-flex align-items-center justify-content-center" data-media-popup data-media-src="{{ $media }}" data-media-type="{{ $mType }}" data-media-title="{{ $row->title??$row->name }}" data-media-badge="{{ ucfirst($type) }} {{ ucfirst($mType) }}" @if($isYt) data-youtube-id="{{ $ytListMatch[1] }}" @endif title="Click to preview {{ $row->title??$row->name }} in pop-up">@if($isYt)<div class="position-relative d-inline-block"><img class="listing-preview" src="https://img.youtube.com/vi/{{ $ytListMatch[1] }}/mqdefault.jpg" alt="{{ $row->title }} preview"><span class="media-preview-play-badge"><i class="bi bi-play-fill"></i></span></div>@elseif($isVideo)<div class="position-relative d-inline-block"><video class="listing-preview video" src="{{ $media }}" muted preload="metadata"></video><span class="media-preview-play-badge"><i class="bi bi-play-fill"></i></span></div>@else<img class="listing-preview" src="{{ $media }}" alt="{{ $row->title??$row->name }} preview">@endif</button>@else<span class="media-placeholder"><i class="bi bi-{{ $type==='speaker'?'person':'image' }}"></i></span>@endif</td>
...
<td>@if($canEdit)<form method="POST" action="{{ route($routeBase.'.toggle',$row) }}">@csrf @method('PATCH')<label class="d-inline-flex align-items-center gap-2 m-0 cursor-pointer"><span class="form-switch m-0"><input class="form-check-input" type="checkbox" @checked($row->is_active) onchange="this.form.submit()" aria-label="Toggle status"></span><strong class="{{ $row->is_active?'text-success':'text-muted' }}">{{ $row->is_active?'Active':'Inactive' }}</strong></label></form>@else<strong class="{{ $row->is_active?'text-success':'text-muted' }}">{{ $row->is_active?'Active':'Inactive' }}</strong>@endif</td>
<td><div class="d-flex gap-2">@if($canEdit)<a class="btn btn-sm btn-light" href="{{ route($routeBase.'.edit',$row) }}">Edit</a>@endif @if($canDelete)<form method="POST" action="{{ route($routeBase.'.destroy',$row) }}" data-confirm-delete data-confirm-title="Delete this {{ $type }}?" data-confirm-text="{{ $row->title??$row->name }} will be permanently deleted.">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger">Delete</button></form>@endif</div></td></tr>
