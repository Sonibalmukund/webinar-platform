...
<form class="panel-card form-panel" method="POST" enctype="multipart/form-data" action="{{ $banner->exists?route('admin.general.banners.update',$banner):route('admin.general.banners.store') }}">@csrf @if($banner->exists)@method('PUT')@endif
    <div class="form-grid">
        ...
        <div class="full" id="bannerImageFields" @if(old('media_type',$banner->media_type)==='video') hidden @endif>
            <label>Choose banner image<input class="form-control" id="bannerImageInput" type="file" name="image_media" accept="image/jpeg,image/png,image/webp"><small>JPG, PNG or WebP, maximum 5 MB.</small></label>
        </div>
        ...
        <div class="full"><div class="banner-live-preview" id="bannerLivePreview">@php($previewMediaSrc=$banner->media_path?:$banner->media_url)@if($previewMediaSrc && $banner->media_type==='image')<button type="button" class="btn p-0 border-0 bg-transparent w-100 h-100 d-block" data-media-popup data-media-src="{{ $previewMediaSrc }}" data-media-type="image" data-media-title="{{ $banner->title ?: 'Banner Preview' }}" data-media-badge="Banner Image" title="Click to view full image in pop-up"><img src="{{ $previewMediaSrc }}" alt="Banner preview" style="max-height: 280px; width: 100%; object-fit: contain;"></button>@elseif($previewMediaSrc && $banner->media_type==='video')@php(preg_match('/(?:youtu\.be\/|youtube(?:-nocookie)?\.com\/(?:watch\?v=|embed\/|shorts\/|live\/))([A-Za-z0-9_-]{11})/i',$previewMediaSrc,$previewYt))@php(preg_match('/vimeo\.com\/(?:video\/)?(\d{6,12})/i',$previewMediaSrc,$previewVim))@if(!empty($previewYt[1]))<iframe src="https://www.youtube-nocookie.com/embed/{{ $previewYt[1] }}?autoplay=1&mute=1&loop=1&playlist={{ $previewYt[1] }}&controls=0" allow="autoplay; encrypted-media" allowfullscreen></iframe>@elseif(!empty($previewVim[1]))<iframe src="https://player.vimeo.com/video/{{ $previewVim[1] }}?autoplay=1&muted=1&loop=1&autopause=0&background=1" allow="autoplay; fullscreen" allowfullscreen></iframe>@else<video src="{{ $previewMediaSrc }}" controls></video>@endif @else<div><i class="bi bi-image"></i><span>Choose a banner file to preview it here.</span></div>@endif</div></div>
    </div>
