...
@section('content')
<div class="page-heading"><div><span class="eyebrow">{{ $subadmin?'ASSIGNED EVENTS':'GLOBAL OVERVIEW' }}</span><h1>Dashboard</h1><p>{{ $subadmin?'Live totals for webinars assigned to you.':'Live platform totals calculated from backend data.' }}</p></div>@if(!$subadmin || auth()->user()->hasPermission('webinars.create'))<a href="{{ route('admin.webinars.create') }}" class="btn btn-gradient"><i class="bi bi-plus-lg"></i> Create webinar</a>@endif</div>
<div class="stats-grid dashboard-metric-grid"><x-stat-card label="Events" :value="$webinars->count()" icon="calendar-event" meta="Total webinars" /><x-stat-card label="Registered users" :value="$registeredUsers" icon="people" tone="purple" meta="All registrations" /><x-stat-card label="New today" :value="$todayRegistrations" icon="person-plus" tone="orange" meta="Today's growth" /><x-stat-card label="Total attendees" :value="$totalAttendees" icon="person-check" tone="blue" meta="Joined sessions" /><x-stat-card label="Live now" :value="$liveNow" icon="broadcast" tone="green" :live="true" meta="Active viewers" /><x-stat-card label="Chat threads" :value="$chatMessages" icon="chat-dots" tone="blue" meta="Messages received" /><x-stat-card label="Comments" :value="$commentsCount" icon="chat-square-text" tone="orange" meta="Private attendee notes" /><x-stat-card label="Polls" :value="$pollsCount" icon="bar-chart" tone="red" meta="Created polls" /><x-stat-card label="Poll voters" :value="$pollVoters" icon="check2-square" tone="green" meta="Unique voters" /><x-stat-card label="Votes submitted" :value="$votesCount" icon="ui-checks-grid" tone="purple" meta="Total responses" /></div>
...
<div class="dashboard-bottom-grid mt-4">
    ...
    <aside class="panel-card">
        <div class="panel-title">
            ...
            @if(! $subadmin || auth()->user()->hasAnyPermission(['registrations.view', 'attendance.view']))
