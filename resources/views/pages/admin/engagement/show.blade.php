...
@forelse($items as $item)
    <article id="{{ $type }}-{{ $item->id }}"><span class="chat-avatar">{{ Str::of($item->user_name?:'G')->substr(0,1)->upper() }}</span><div class="voice-content"><div><span><strong>{{ $item->user_name?:'Guest user' }}</strong><small>{{ $item->user_email }} · {{ Carbon\Carbon::parse($item->created_at)->diffForHumans() }}</small></span>@if($type==='feedback')<div class="voice-stars">@for($i=1;$i<=5;$i)<i class="bi bi-star{{ $i<=($item->rating??0)?'-fill':'' }}"></i>@endfor</div>@endif</div><p>{{ $type==='feedback'?$item->message:$item->comment }}</p>@if($type==='comments')<footer><form method="POST" action="{{ route('admin.comments.destroy',[$webinar,$item->id]) }}" onsubmit="return confirm('Remove this comment?')">@csrf @method('DELETE')<button class="btn btn-sm btn-light text-danger"><i class="bi bi-trash3"></i> Remove</button></form></footer>@endif</div></article>
@empty
