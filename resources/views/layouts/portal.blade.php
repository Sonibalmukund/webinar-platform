...
@section('toast')
@php($flashMessage = session('auth_status') ?: session('registration_status') ?: session('status') ?: session('success'))
@if($flashMessage)<span hidden data-app-flash="{{ $flashMessage }}"></span>@endif
<div class="toast-container position-fixed top-0 end-0 p-3" style="z-index:3000"><div id="appToast" class="toast border-0 shadow" role="status" aria-live="polite"><div class="toast-body d-flex gap-2"><i class="bi bi-check-circle-fill text-success"></i><span>{{ $flashMessage }}</span></div></div></div>
@endsection
